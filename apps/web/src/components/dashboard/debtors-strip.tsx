...
import { formatRub, getInitials } from '@tutorflow/utils'
    ...
    <section className="h-full rounded-lg bg-card border border-[var(--border-subtle)] shadow-elevation-1 p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">{t('debtors.title')}</h3>
        </div>
      ...
      {debtors.length > 1 && (
        <div className="mt-3 pt-3 flex items-center justify-between border-t border-[var(--border-subtle)]">
          <span className="text-xs text-muted-foreground">
            {t('total')}: <span className="font-semibold text-warning tnum">{formatRub(total)}</span>
          </span>
          {debtors.length > 6 && (
            <Link href="/finances" className="flex items-center text-xs text-primary font-medium gap-0.5">
              {t('debtors.morePeople', { count: debtors.length - 6 })} <ChevronRight size={14} />
            </Link>
          )}
        </div>
      )}
