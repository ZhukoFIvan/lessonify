  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className={`w-11 h-11 rounded-full flex items-center justify-center shrink-0 ${isPro ? 'bg-amber-500/10' : 'bg-primary/10'}`}>
          {isPro ? <Crown size={22} className="text-amber-500" /> : <Zap size={22} className="text-primary" />}
        </div>
        ...
        <span className={`text-sm font-bold px-3 py-1 rounded-full ${isPro ? 'bg-amber-500/10 text-amber-600' : 'bg-secondary text-muted-foreground'}`}>
          {isPro ? 'PRO' : 'FREE'}
        </span>
      </div>
      ...
      <div className="space-y-2.5">
        {(isPro ? PRO_FEATURES : FREE_FEATURES).map((f) => (
          <div key={f} className="flex items-center gap-2.5">
            <Check size={15} className={isPro ? 'text-amber-500 shrink-0' : 'text-primary shrink-0'} />
        ...
        <div className="space-y-2">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl bg-secondary/50 border border-border text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 transition-colors"
            />
            <button
      ...
      {!isPro && (
        <div className="space-y-2">
          <p className="text-sm font-medium text-foreground">Тарифы PRO</p>
          <div className="grid grid-cols-2 gap-3">
            {/* Monthly */}
            <button
              onClick={() => checkout('monthly')}
              disabled={billingLoading}
              className="flex flex-col items-center gap-1.5 p-4 rounded-xl border border-border hover:border-primary/50 transition-colors disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={billingLoading}
              className="flex flex-col items-center gap-1.5 p-4 rounded-xl border border-primary/30 bg-primary/5 hover:border-primary/50 transition-colors relative disabled:opacity-50"
            >
              <span className="absolute -top-2 right-2 text-xs font-bold px-2 py-0.5 rounded-full bg-green-500/10 text-green-600">
                −33%
              </span>
      ...
      {isPro && (
        <div className="space-y-2">
          <p className="text-sm font-medium text-foreground">Продлить PRO</p>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => checkout('monthly')}
              disabled={billingLoading}
              className="flex flex-col items-center gap-1.5 p-4 rounded-xl border border-border hover:border-amber-500/50 transition-colors disabled:opacity-50"
            >
              <span className="text-2xl font-bold text-foreground">499 ₽</span>
              <span className="text-sm text-muted-foreground">+1 месяц</span>
            </button>
            <button
              onClick={() => checkout('yearly')}
              disabled={billingLoading}
              className="flex flex-col items-center gap-1.5 p-4 rounded-xl border border-amber-500/30 bg-amber-500/5 hover:border-amber-500/50 transition-colors relative disabled:opacity-50"
            >
              <span className="absolute -top-2 right-2 text-xs font-bold px-2 py-0.5 rounded-full bg-green-500/10 text-green-600">
                −33%
              </span>
              <span className="text-2xl font-bold text-foreground">3 990 ₽</span>
              <span className="text-sm text-muted-foreground">+1 год</span>
            </button>
          </div>
          {billingError && <p className="text-xs text-destructive mt-1">{billingError}</p>}
        </div>
